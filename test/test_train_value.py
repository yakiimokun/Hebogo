...
from scripts.train_value import _encode_boards, main
...
class ValueTrainingTest(unittest.TestCase):
    ...
    def _sgf(self, name: str, first_move: str, result="B+R"):
        (self.input_dir / name).write_text(
            f"(;GM[1]SZ[9]RE[{result}];B[{first_move}];W[bb])",
            encoding="utf-8",
        )
    ...
    def test_channel_order_matches_encoder(self):
    ...
    def test_two_games_save_loadable_state_dict(self):
        self._sgf("black.sgf", "aa", "B+R")
        self._sgf("white.sgf", "cc", "W+R")
        ...
        model = ValueNetwork(9)
        model.load_state_dict(torch.load(self.output, map_location="cpu", weights_only=True))
        with torch.no_grad():
            value = model(torch.zeros(1, 3, 9, 9))
        self.assertEqual(value.shape, (1, 1))

    def test_one_game_needs_validation_disabled(self):
