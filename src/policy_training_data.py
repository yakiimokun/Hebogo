"""Policy Network の教師あり学習用に SGF を棋譜単位で分割する。"""
...
def _arrays(
    ...
    moves = np.asarray([sample.move for sample in samples], dtype=np.int64)
    return boards, moves
...
def load_policy_splits(
    ...
    train_boards, train_moves = _arrays(train_games, board_size)
    validation_boards, validation_moves = _arrays(validation_games, board_size)
    return PolicyDataSplit(
        train_boards=train_boards,
        train_moves=train_moves,
        validation_boards=validation_boards,
        validation_moves=validation_moves,
